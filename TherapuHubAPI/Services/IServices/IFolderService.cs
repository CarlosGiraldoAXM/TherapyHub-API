...
public interface IFolderService
{
    ...
    Task<bool> DeleteFolderAsync(int id, int companyId, int actorId, int userTypeId);

    // Files
    Task<IEnumerable<FileResponseDto>> GetFilesAsync(int folderId, int companyId);
    Task<FileResponseDto> UploadFileAsync(int folderId, IFormFile file, int companyId, int actorId);
    Task<(Stream Stream, string ContentType, string FileName)?> DownloadFileAsync(long fileId, int folderId, int companyId);
    Task<bool> DeleteFileAsync(long fileId, int folderId, int companyId, int deleterActorId);
}
